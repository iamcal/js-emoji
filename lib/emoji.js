...
;(function() {
	...
	emoji.prototype.replacement = function(idx, actual, wrapper, variation){
		...
		var img = self.data[idx][7] || self.img_sets[self.img_set].path+idx+'.png';
		...
		if (variation_idx && self.variations_data[variation_idx] && self.variations_data[variation_idx][2] && !self.data[idx][9]){
		...
		if (self.supports_css) {
			if (self.use_sheet && px != null && py != null){
				...
				var style = 'background: url('+self.img_sets[self.img_set].sheet+');background-position:'+(mul*px)+'% '+(mul*py)+'%;background-size:'+self.sheet_size+'00%';
